...
void AXPlatformNodeWin::FireUiaTextEditTextChangedEvent(
  ...
  // TODO(gw280): When we split out FML into a standalone library, switch
  // this to use scoped RAII wrappers around BSTR/SAFEARRAY.

  // Composition has been finalized by TSF
  BSTR composition_text = ::base::String16ToBSTR(active_composition_text);
  SAFEARRAY* changed_data =
      ::SafeArrayCreateVector(VT_BSTR /* element type */, 0 /* lower bound */,
                            1 /* number of elements */);
  if (!changed_data) {
    ::SysFreeString(composition_text);
    ::SafeArrayDestroy(changed_data);
    return;
  }

  LONG index = 0;
  HRESULT hr =
      SafeArrayPutElement(changed_data, &index, composition_text);
...
IFACEMETHODIMP AXPlatformNodeWin::get_accDefaultAction(VARIANT var_id,
  ...
  *def_action = base::String16ToBSTR(action_verb);
...
IFACEMETHODIMP AXPlatformNodeWin::get_accName(VARIANT var_id, BSTR* name_bstr) {
  ...
  *name_bstr = base::String16ToBSTR(name);
  return S_OK;
}
...
HRESULT AXPlatformNodeWin::GetPropertyValueImpl(PROPERTYID property_id,
  ...
  switch (property_id) {
    case UIA_AriaPropertiesPropertyId:
      result->vt = VT_BSTR;
      result->bstrVal = base::String16ToBSTR(ComputeUIAProperties());
      break;

    case UIA_AriaRolePropertyId:
      result->vt = VT_BSTR;
      result->bstrVal = base::String16ToBSTR(UIAAriaRole());
      break;

    case UIA_AutomationIdPropertyId:
      V_VT(result) = VT_BSTR;
      V_BSTR(result) =
          base::String16ToBSTR(GetDelegate()->GetAuthorUniqueId());
      break;
    ...
    case UIA_LocalizedControlTypePropertyId: {
      ...
      if (!localized_control_type.empty()) {
        result->vt = VT_BSTR;
        result->bstrVal = base::String16ToBSTR(localized_control_type);
      }
    ...
    case UIA_LocalizedLandmarkTypePropertyId: {
      ...
      if (!localized_landmark_type.empty()) {
        result->vt = VT_BSTR;
        result->bstrVal = base::String16ToBSTR(localized_landmark_type);
      }
      break;
    }
  ...
  if (property_id ==
    ...
    result->vt = VT_BSTR;
    result->bstrVal =
        base::String16ToBSTR(base::NumberToString16(-GetUniqueId()));
  }
...
BSTR AXPlatformNodeWin::GetValueAttributeAsBstr(AXPlatformNodeWin* target) {
  ...
  if (target->GetData().role == ax::mojom::Role::kColorWell) {
    ...
    BSTR value = base::String16ToBSTR(value_text);
    BASE_DCHECK(value);
    return value;
  }
  ...
  if (target->GetData().role == ax::mojom::Role::kRootWebArea ||
    ...
    BSTR value = base::String16ToBSTR(result);
    BASE_DCHECK(value);
    return value;
  }
...
HRESULT AXPlatformNodeWin::GetStringAttributeAsBstr(
  ...
  *value_bstr = base::String16ToBSTR(str);
...
BSTR AXPlatformNodeWin::GetFontNameAttributeAsBSTR() const {
  ...
  return base::String16ToBSTR(string);
}
...
BSTR AXPlatformNodeWin::GetStyleNameAttributeAsBSTR() const {
  ...
  return base::String16ToBSTR(style_name);
}
